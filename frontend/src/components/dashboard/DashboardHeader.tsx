...
export default function DashboardHeader({
  ...
  return (
    <>
      <header className="bg-white border-b border-border/80 px-4 sm:px-6 md:px-8 py-3.5 sm:py-4 w-full">
        ...
        <div className="flex items-center justify-between text-xs sm:text-sm text-text-muted mb-2.5">
          <div className="flex flex-wrap items-center gap-1.5 font-medium min-w-0">
            <button onClick={onBackToLanding} className="hover:text-nexus-600 transition cursor-pointer shrink-0">
              Home
            </button>
            <span>&rsaquo;</span>
            <span className="shrink-0">Repositories</span>
            {hasRepositories && (
              <>
                <span>&rsaquo;</span>
                <span className="text-text-primary font-semibold truncate max-w-[140px] sm:max-w-none">{repoName}</span>
                {analysis?.id && (
                  <>
                    <span>&rsaquo;</span>
                    <span className="text-text-secondary font-mono shrink-0">Analysis #{analysis.id.substring(0, 6)}</span>
                  </>
                )}
              </>
            )}
          ...
          {/* User profile avatar (Visible on desktop breadcrumb) */}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
          <div className="space-y-2 min-w-0">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3.5">
              ...
              <div className="relative inline-block text-left" ref={dropdownRef}>
                <button
                  ...
                  className="group inline-flex items-center space-x-1.5 sm:space-x-2 text-xl sm:text-2xl md:text-3xl font-extrabold text-text-primary tracking-tight hover:text-nexus-600 transition cursor-pointer rounded-lg px-1.5 py-0.5 -ml-1.5 hover:bg-slate-100/70 max-w-full"
                  aria-expanded={isDropdownOpen}
                  aria-haspopup="true"
                >
                  <span className="truncate max-w-[200px] xs:max-w-[260px] sm:max-w-none">{repoName}</span>
                  <svg
                    className={`w-5 h-5 text-text-muted group-hover:text-nexus-600 transition-transform duration-200 shrink-0 ${
            ...
            {hasRepositories && analysis && (
              <div className="flex flex-wrap items-center gap-x-3 sm:gap-x-5 gap-y-1 text-xs sm:text-sm text-text-muted break-words">
                {analysis.repository?.clone_url && (
                  <span className="flex items-center space-x-1.5 font-medium text-text-secondary">
                    ...
                    <span className="truncate max-w-[160px] xs:max-w-[200px] sm:max-w-none">{analysis.repository.clone_url.replace('https://github.com/', '')}</span>
                  </span>
                )}
                ...
                <span className="break-all">Analysis ID: <strong className="font-mono text-text-secondary">{`NEXUS-${analysis.id.substring(0, 8)}`}</strong></span>
